...
node_t* scope_insert(scope_t* top, char* name){
    ...
    top->table[indexToInsert] = insertNameNode(nodeToInsert, name);  
}

node_t* scope_insert_var(scope_t* top, int type, char* name){
    int indexToInsert = hashpjw(name);
    node_t* nodeToInsert = top->table[indexToInsert];
    top->table[indexToInsert] = insertVarNode(nodeToInsert, type, name);  
}

node_t* scope_insert_array(scope_t* top, int type, char* name, int dimension){
    int indexToInsert = hashpjw(name);
    node_t* nodeToInsert = top->table[indexToInsert];
    top->table[indexToInsert] = insertArrayNode(nodeToInsert, type, name, dimension);  
}

node_t* scope_insert_procedure(scope_t* top, char* name, node_t* arguments){
    int indexToInsert = hashpjw(name);
    node_t* nodeToInsert = top->table[indexToInsert];
    top->table[indexToInsert] = insertProcedureNode(nodeToInsert, name, arguments);  
}

node_t* scope_insert_function(scope_t* top, int returnType, char* name, node_t* arguments){
    int indexToInsert = hashpjw(name);
    node_t* nodeToInsert = top->table[indexToInsert];
    top->table[indexToInsert] = insertFunctionNode(nodeToInsert, returnType, name, arguments);  
}

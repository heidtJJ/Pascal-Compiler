...
node_t *insertArrayNode(node_t *top, int type, char *name, int start, int end)
{
    if (start > end)
    {
        yyerror("Array start index must be less than array end index.");
    }
...
/*
    Returns 1 is the top is an array node.
*/
short isArrayNode(node_t *top)
{
    if (top == NULL || top->nodeType != VAR)
    {
        return 0;
    }
    return top->data.varInfo.isArray;
}
...
void copyNode(node_t *source, node_t *destination)
{
    ...
    { /* Does not support subprograms yet */
        assert(0);
    }
}

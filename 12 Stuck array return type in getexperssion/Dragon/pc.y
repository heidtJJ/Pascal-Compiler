    ...
    : BBEGIN optional_statements END 
        { 
            ...
            if(scopeOwner->nodeType == FUNCTION){
                flags = NO_FUNCTION_RETURN | NO_SIDE_EFFECTS;
            }
            
            fprintf(stderr, "fsfdsafdsfsafdsfdsfsadfasd\n\n\n\n\n");
            validateStatement(top_scope, statementTree, &flags);
            
            fprintf(stderr, "After validateStatement %X\n", flags);

            if(scopeOwner->nodeType == FUNCTION){
                fprintf(stderr, "After validateStatement %X   %X  %X \n", flags, flags & NO_FUNCTION_RETURN , flags & ~NO_SIDE_EFFECTS);
...
variable
    ...
    { 
        node_t* matchNode = scope_search_all(top_scope, $1);
        if(!matchNode || matchNode->nodeType != ARRAY){

...
int getExpressionType(scope_t* curScope, tree_t* exprTree){
    ...
    if(exprTree->type == RELOP){
        ...
        node_t* nodeMatch = scope_search_all(curScope, exprTree->attribute.nval->name);
        if(!nodeMatch){
        ...
        char* arrayId = exprTree->leftChild->attribute.nval->name;
        node_t* nodeMatch = scope_search_all(curScope, arrayId);
        if(!nodeMatch){
            yyerror("ARRAY_ACCESS ID error in getExpressionType");
        }
        else{
            if(nodeMatch->nodeType != ARRAY){
                fprintf(stderr, "Bitch ass: %s %d\n", nodeMatch->name, nodeMatch->data.type);
...
void validateStatement(scope_t* curScope, tree_t* exprTree, unsigned int* flags){
    if(exprTree == NULL){
        return;
    }
    if(exprTree->type == ASSIGNOP){
        ...
        if(scope_search(curScope, variableName) == NULL){
            ...
            *flags = *flags & ~NO_SIDE_EFFECTS;
            fprintf(stderr, "In validateStatement are side effects %X\n\n", *flags);
         }        
    }

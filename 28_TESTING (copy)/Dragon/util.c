...
void validateStatement(scope_t* curScope, tree_t* stmtTree, int* flags){
    ...
    if(stmtTree->type == ASSIGNOP){
        ...
        // Left Child ival is a value indicating side effects in the
        // compound statement. Compound statement already checked for
        // return value setting (compound_statement in pc.y). 
        if(stmtTree->leftChild->attribute.ival){
            // Changing variable outside of scope in this 
            // compound statement. Indicate this in flags.
            *flags = *flags & ~NO_SIDE_EFFECTS;
            fprintf(stderr, "In COMPOUND_STATEMENT validateStatement %s are side effects %X\n\n", curScope->scopeOwner->name, *flags);
        }
    }
    else{
